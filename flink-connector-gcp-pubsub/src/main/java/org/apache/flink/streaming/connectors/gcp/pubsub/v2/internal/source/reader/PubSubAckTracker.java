...
/** In-memory {@link AckTracker} that holds acks per checkpoint until that checkpoint completes. */
public class PubSubAckTracker implements AckTracker {
    ...
    public synchronized void nackAll() {
        ...
        checkpoints
                ...
                .forEach(
                        (ackReplyConsumers) ->
                                ackReplyConsumers.forEach(
                                        (ackReplyConsumer) -> ackReplyConsumer.nack()));
        checkpoints.clear();
    }
}
